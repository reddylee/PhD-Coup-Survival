...
mydata2 <- mydata1 |> 
  mutate(Year = ifelse(is.na(overstay_year), syear, overstay_year),
         age = ifelse(is.na(overstay_year), syear-yrborn, overstay_year-yrborn)) |>
  select(ccode, idacr, Year, leader, age, group, survival_time, status) |>
  add_row(ccode = 552, 
          idacr = "ZIM", 
          Year = 2017, 
...
mydata3 <- mydata2 |> 
  ...
  drop_na() |> 
  select(ccode, idacr, country, year = Year, leader, age, group, survival_time, status, GDP_growth_trend, GDP_pc_log, pop_log, polity5, polstab) |> 
  filter(survival_time > 180 & survival_time < 18000)

table(mydata3$group)
table(mydata3$status)
```
```{r}
write.csv(mydata3, "mydata.csv")
```
