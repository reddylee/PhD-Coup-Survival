...
mydata4 <- mydata3 |> 
  ...
  drop_na() |> 
  select(id, ccode, idacr, country, tstart, tstop, leader, age, group, survival_time, status, GDP_growth_trend, GDP_pc_log, pop_log, polity5, polstab)

table(mydata4$group)
table(mydata4$status)
```
```{r}
mydata5 <- mydata2 |> 
  ...
  drop_na() |> 
  select(ccode, idacr, country, year, leader, age, group, survival_time, status, GDP_growth_trend, GDP_pc_log, pop_log, polity5, polstab)

table(mydata5$group)
table(mydata5$status)
```
...
mydata6 <- mydata2 |>
  ...
  mutate(year = first(year(startdate)) + row_number() - 1,
         .by = id) |>
  select(id, ccode, idacr, year, leader, age, group, tstart, tstop, status)
...
mydata7 <- mydata6 |>
  ...
  left_join(polstab, by = c("ccode", "year" = "year")) |>
  drop_na() |>
  select(id, ccode, idacr, year, leader, age, group, tstart, tstop, status, GDP_growth_trend, GDP_pc_log, pop_log, polity5, polstab)

```

```{r}
write.csv(mydata5, "mydata1.csv")
write.csv(mydata4, "mydata2.csv")
write.csv(mydata7, "mydata3.csv")
```
