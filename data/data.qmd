...
#   view()
gdp <- vdem |> 
  select(ccode = COWcode, country = country_name, year, GDP = e_gdp, GDP_pc = e_gdp, population = e_pop) |>
  filter(year >= 1945)|> 
...
GDP <- gdp |> 
  ...
  select(ccode, country, year, GDP_growth_trend, GDP_pc_log) |>
  drop_na() 
```
...
```{r}
mydata2 <- mydata1 |> 
  mutate(Year = ifelse(is.na(overstay_year), syear, overstay_year)) |>
  select(ccode, idacr, Year, leader, group, survival_time, status) |>
  add_row(ccode = 552, 
          idacr = "ZIM", 
          Year = 2017, 
          leader = "Emmerson Mnangagwa",
          group = "B",
          survival_time = ymd(20231127)-ymd(20171124),
          status = 1) |> # add an observation for a new leader
  left_join(GDP, by = c("ccode", "Year" = "year")) |> 
  left_join(polity, by = c("ccode", "Year" = "year")) |>
  drop_na() |> 
  select(ccode, idacr, country, year = Year, leader, group, survival_time, status, GDP_growth_trend, GDP_pc_log, polity) |> 
