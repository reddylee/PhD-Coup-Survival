...
```{r}
#| label: fig-coxSurv
#| fig-cap: Cox proportional hazards model for survival time of leaders

plot_data <- data.frame(
  group = c("Overstay leaders", "Coup-entry leaders"),
  GDP_growth_trend = mean(Mydata$GDP_growth_trend),
  GDP_pc_log = mean(Mydata$GDP_pc_log),
  pop_log = mean(Mydata$pop_log),
  polity5 = mean(Mydata$polity5),
  polstab = mean(Mydata$polstab),
  age = mean(Mydata$age)
)
