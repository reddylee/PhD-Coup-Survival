...
#| fig-height: 5
#| fig-align: center
#| fig-cap: Kaplan-Meier plot for survival curves of overstay and coup-entry leaders
...
Specifically, according to @fig-logrank, the average survival time post-overstay, excluding their original term duration, is approximately `r avg$avg[1]` years.
In contrast, leaders who gain power via coups tend to have an average survival time of `r avg$avg[2]` years, representing an average shortfall of 5 years in their tenure [^1].
...
| **Aspect**                       | **Overstay Leaders**                                                                                    | **Coup-Entry Leaders**                                                                                                                                               |
|------------------|----------------------|--------------------------------|
...
coups |>
  ...
  footnote(general = "",
           general_title = "Data source: Powell & Thyne(2011)")
```
...
Here, I employ Cox proportional hazards models to assess the likelihood of leaders being removed from office, essentially examining the hazards associated with leadership tenure termination.
...
Mydata1 |>
  mutate(group = factor(group),
         survt = tstop - tstart) |>
  coxph(
    Surv(survt, status) ~ group + GDP_growth_trend + GDP_pc_log + pop_log + polity5 + polstab + age,
    data = _
  ) |> 
...
Mydata2 |>
  mutate(group = factor(group),
         survt = tstop - tstart) |>
  coxph(
    Surv(tstart, tstop, status) ~ group + GDP_growth_trend + GDP_pc_log + pop_log + polity5 + polstab + age + cluster(id),
    data = _
  ) |> 
...
tbl_merge(tbls = list(tbl1, tbl2)) |> 
  ...
  as_gt()
```
Based on the previous discussion, I test my hypothesis that overstaying leaders are more likely to experience extended regime survival compared to leaders who come to power through coups. The regression outcome of cox proportional hazards model is presented in @tbl-cox. The @tbl-cox underscores the significance of leader type and GDP per capita in influencing the hazard ratios associated with leaders' ousting. Specifically, the hazard ratio for coup-entry leaders is 2.49, signifying that coup-entry leaders are 1.49 times more likely to face ousting compared to overstaying leaders, all else being equal. Additionally, the hazard ratio associated with the logarithm of GDP per capita is 0.73, suggesting that a 1% increase in GDP per capita is correlated with a 27% reduction in the hazard of being ousted. It is noteworthy that the remaining variables did not exhibit statistical significance in this analysis.

```{r}
#| label: fig-coxHR
#| fig-cap: Hazard ratios and 95% CIs for Leader Ousting

coxplottidy <- tidy(model_cox, exponentiate = T, conf.int = T)
...
coxplottidy |>
  ...
  theme_bw() 
```
The @fig-coxHR illustrates the hazard ratios and 95% confidence intervals for the variables in the Cox model. The hazard ratio for coup-entry leaders is 2.49, signifying that coup-entry leaders are 1.49 times more likely to face ousting compared to overstaying leaders, all else being equal. Additionally, the hazard ratio associated with the logarithm of GDP per capita is 0.73, suggesting that a 1% increase in GDP per capita is correlated with a 27% reduction in the hazard of being ousted. It is noteworthy that the remaining variables did not exhibit statistical significance in this analysis.

```{r}
summary(model_cox)
names(model_cox)
tidy(survfit(model_cox),newdata = plot_data)
```
```{r}
model_reduced <- Mydata |>
  mutate(group = factor(group, labels = c("Overstay leaders", "Coup-entry leaders"))) |>
  coxph(
    Surv(survival_time, status) ~ group + GDP_pc_log,
    data = _
  )
...
```{r}
anova(model_reduced, model_cox, test = "LRT")
```

```{r}
plot(predict(model_cox),residuals(model_cox, type = "deviance"))
abline(h = 0)
lines(smooth.spline(predict(model_cox),residuals(model_cox, type = "deviance")),
                    col = "red")
```

```{r}
plot(predict(model_cox),residuals(model_cox, type = "martingale"))
abline(h = 0)
lines(smooth.spline(predict(model_cox),residuals(model_cox, type = "martingale")),
                    col = "red") 
```
```{r}
cox.zph(model_cox)
...
```{r}
survdiff(Surv(survival_time, status) ~ group, data = Mydata)
```

```{r}
#| label: fig-cloglog
#| fig-cap: "The cumulative hazard plot by leader type"

plot_data <- Mydata |> 
  mutate(group = factor(group, labels = c("Overstay leaders", "Coup-entry leaders"))) |> 
  select(GDP_growth_trend, GDP_pc_log, pop_log, polity5, polstab, age, group) |> 
  summarise(
    across(everything(), mean, na.rm = TRUE), .by = group) |> 
  mutate(GDP_growth_trend = 0,
         polity5 = 0)

model_cox |> 
  survfit(newdata = plot_data) |>
  ggsurvplot(
    data = plot_data,
    fun = "cloglog",
...
The @fig-cloglog illustrates the the cumulative hazard curves for overstaying leaders and coup-entry leaders. The cumulative hazard curve for overstaying leaders is below that of coup-entry leaders, indicating that overstaying leaders are less likely in hazard compared to leaders who come to power through coups.

```{r}
#| label: fig-coxSurv
#| fig-cap: Cox proportional hazards model for survival probability by leader type
...
plot2 <- Mydata |>
  mutate(group = factor(group)) |>
  coxph(
    Surv(survival_time, status) ~ strata(group) + GDP_growth_trend + GDP_pc_log + pop_log + polity5 + polstab + age,
    data = _
  ) |> 
  survfit(newdata = testdata) |>
  ggsurvplot(data = testdata,
             surv.median.line = "hv",
             conf.int = TRUE,
             break.time.by = 2000,
             ggtheme = theme_bw())
...
The @fig-coxSurv illustrates the survival curves for overstaying leaders and coup-entry leaders. The survival probability of overstaying leaders is significantly higher than that of coup-entry leaders, as the survival probability of overstaying leaders is 0.85 at 5,000 days, while that of coup-entry leaders is 0.65 at the same time point. This finding is consistent with the results of the Cox model, which indicates that overstaying leaders are more likely to experience extended regime survival compared to leaders who come to power through coups.
