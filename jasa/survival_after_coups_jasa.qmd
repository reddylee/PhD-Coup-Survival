...
Based on the previous discussion, I test my hypothesis that overstaying leaders are more likely to experience extended regime survival compared to leaders who come to power through coups. The regression outcome of cox proportional hazards model is presented in @tbl-cox.
...
tbl_merge(tbls = list(plot1, plot2)) |>
  modify_spanning_header(everything() ~ NA_character_) |>
  as_kable_extra(format = "latex",
                 booktabs = T) |>
  kable_styling(latex_options = "striped",
                position = "center") |>
  column_spec(column = 1:6,
              width = c("4cm", "2cm", "2cm", "1.5cm", "2.5cm","1.5cm")) |>
  collapse_rows(latex_hline = "linespace")

```

```{r}
coxplottidy <- tidy(coxplot, exponentiate=T, conf.int=T)
coxplottidy |> 
  gt()
```
